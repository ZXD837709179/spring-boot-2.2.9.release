...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // Packing and unpacking ctl 状态，取前三位
    private static int runStateOf(int c)     { return c & ~CAPACITY; }

	//工作线程数，取后面的28位，包括核心线程和非核心线程
        ...
        implements Runnable
    {
		...
		//将要运行的初始任务
        Runnable firstTask;
        /** Per-thread task counter 每个线程的任务计数器 */
...
//        handler.rejectedExecution(command, this);
     ...
     */
	//创建线程，可能是核心线程，也可能是非核心的
	//核心的直接执行？非核心的firstTask为空，创建后从队列拿
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            if (rs >= SHUTDOWN &&
				...
				//（2）线程池状态为SHUTDOWN，且要执行的任务不为空，且任务队列为空
                return false;
			//cas自旋更新线程池线程数
            for (;;) {
    ...
    private Runnable getTask() {
        ...
        for (;;) {
			...
			// 当前线程数>最大线程数 或 （允许超时销毁并且超时） 并且 （当前线程数>1 或 工作队列为空），减少工作线程数并返回null
            ...
            try {
                ...
                if (r != null)
                    return r;
				//核心线程如果在队列找不到任务，这里也会置为true，在下轮循环将会compareAndDecrementWorkerCount
                timedOut = true;
    ...
    public void execute(Runnable command) {
        ...
        if (isRunning(c) && workQueue.offer(command)) {
			//再次检查线程池状态，如果线程池状态不是RUNNING，则从阻塞队列中移除任务，并拒绝任务
            int recheck = ctl.get();
            if (! isRunning(recheck) && remove(command))
                reject(command);
			//当线程池中的workerCount为0时，此时workQueue中还有待执行的任务，则新增一个addWorker，消费workqueue中的任务
			//能放入队列中但是线程数却是0？？可能设置的核心线程为0，这种情况队列不为空但是永远不能执行
